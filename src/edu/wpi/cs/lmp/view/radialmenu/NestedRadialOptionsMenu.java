...
public class NestedRadialOptionsMenu extends RadialOptionsMenu {
	...
	public NestedRadialOptionsMenu(NestedRadialOptionsMenuDemo caller, final String[] itemNames, final HashMap<String, List<String>> nestedListMap, final double innerRadius,
	...
	private final class ItemOnEventHandler implements EventHandler<MouseEvent> {
		...
		public void handle(final MouseEvent event) {
			if (event.getEventType() == MouseEvent.MOUSE_ENTERED) {
				...
				if (Math.abs(colorItem.getInnerRadius() + colorItem.getOffset()
						- distanceToCenter) < 20) {
